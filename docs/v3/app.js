document.addEventListener('DOMContentLoaded', () => {
    ...
    const exportToClipboardBtn = document.getElementById('export-to-clipboard-btn'); // 追加
    const importFromClipboardBtn = document.getElementById('import-from-clipboard-btn'); // 追加
    ...
    let saveTimer = null;
    const GIT_COMMIT_HASH = '1783cd1c06443539e84ab7c709df7bb52589dd90';
    const GIT_COMMIT_MESSAGE = 'C-Style  ';
    ...
    const exportToClipboard = async () => {
        try {
            ...
            alert('LocalStorageのデータがクリップボードにコピーされました。');
        } catch (err) {
            console.error('クリップボードへの書き込みに失敗しました:', err);
            alert('クリップボードへの書き込みに失敗しました。ブラウザの権限を確認してください。');
        }
        menuDropdown.classList.remove('show');
    };

    // クリップボードからデータをインポートする関数
    const importFromClipboard = async () => {
        try {
            const text = await navigator.clipboard.readText();
            const data = JSON.parse(text);
            if (confirm('クリップボードのデータでLocalStorageを上書きしますか？現在のデータは失われます。')) {
                for (const key in data) {
                    localStorage.setItem(key, data[key]);
                }
                alert('LocalStorageのデータがクリップボードからインポートされました。ページをリロードします。');
                location.reload();
            }
        } catch (err) {
            console.error('クリップボードからの読み込みに失敗しました:', err);
            alert('クリップボードからの読み込みに失敗しました。有効なJSONデータがクリップボードにありません、またはブラウザの権限を確認してください。');
        }
        menuDropdown.classList.remove('show');
    };
    ...
    exportToClipboardBtn.addEventListener('click', exportToClipboard);
